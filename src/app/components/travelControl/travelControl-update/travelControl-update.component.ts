...
@Component({
    ...
    templateUrl: './travelControl-update.component.html',
    styleUrls: ['./travelControl-update.component.css']
  })
  export class TravelControlUpdateComponent implements OnInit {
  ...
  travelDateForm: FormControl = new FormControl(null);   
  ...
  constructor(
    ...
    ngOnInit(): void {
        ...
        this.travelControl.id = this.activedRoute.snapshot.paramMap.get('id'); 
        this.findbyId(); 
    }

    findbyId(): void{
        this.travelControlService.findById(this.travelControl.id).subscribe(resposta =>{      
            this.travelControl = resposta;
            var [dia, mes, ano] = this.travelControl.travelDate.split('/');
            this.travelDateStart = new Date(Number(ano), Number(mes) - 1, Number(dia)); 
            
            this.selectMember(this.travelControl.member.id, this.travelControl.member.firstName + 
                ' - ' + this.travelControl.member.lastName, this.travelControl.member.headQuarter.description);
                this.KmControl(false, this.travelControl.kmControl)        
              })
    }
    ...
    findAllMember(){
      ...
      update(): void{  
        if(this.travelControl.member.id != null) {
            this.travelControlService.update(this.travelControl).subscribe(() =>{      
              this.toastr.success('Viagem atualizada com Sucesso!', 'Update');
